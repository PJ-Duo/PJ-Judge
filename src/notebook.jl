...
# ╔═╡ 5ecb1376-7557-433d-a090-3f41f938fb53
using Flux, JSON

# ╔═╡ ac80d0b1-83c6-4f4a-bdac-1c0efce2ec14
# Load the JSON data
data = JSON.parsefile("../data/data.json")

# ╔═╡ 50f588a7-78bd-464f-aab7-66bace9004c5
# Extract the tags, patterns, and responses
tags = [intent["tag"] for intent in data["intents"]]

# ╔═╡ 0ef14516-7207-4b4d-861d-c6b3db809762
patterns = [intent["patterns"] for intent in data["intents"]]

# ╔═╡ e92da055-854f-4d52-9969-bfe7bdbbcfd4
responses = [intent["responses"] for intent in data["intents"]]

# ╔═╡ 51fbe9c5-5fb7-4f9d-8872-e48040419c27
# Create a list of all words used in the patterns
words = vcat(patterns)

# ╔═╡ e25c19ec-032d-4276-a1c7-aa6efb778a7a
# Create a dictionary of word-to-index mappings
word_to_index = Dict(word => i for (i, word) in enumerate(words))

# ╔═╡ 9b3af1c1-8eb0-4f10-8e60-a625dd1d328e
# Define a function to convert a pattern into a bag-of-words vector
function pattern_to_vector(pattern)
    vec = zeros(length(words))
    for word in pattern
		for key in keys(word_to_index)
    		if word in key
        		vec[word_to_index[key]] = 1
        		break
    		end
		end
    end
...
# ╔═╡ 5fcf0262-3e4e-4975-a1ea-bdd5d574e7a5
# Convert all patterns into bag-of-words vectors
vectors = [pattern_to_vector(pattern) for pattern in patterns]

# ╔═╡ 55b9aba6-5c76-466d-915d-a781d82a022f
# Define the model architecture
model = Chain(Dense(length(words), 16, softplus),
              Dense(16, length(tags)))

# ╔═╡ 9ef68f72-cbed-4d9c-9dbf-27cf50cf2947
# Define the loss function
loss(x, y) = Flux.mse(model(x), y)

# ╔═╡ fb79b843-b08e-4dc3-8469-8d0457612226
# Define the training data
train_data = [(vector, Flux.onehot(tag, tags)) for (vector, tag) in zip(vectors, tags)]

# ╔═╡ 4c9e0905-b2d4-43d3-861c-f63d5fd92b17
# Train the model
Flux.train!(loss, Flux.params(model), train_data, ADAM())

# ╔═╡ ad68a736-4748-4ede-8399-47ffeaf74b2d
begin
	A = Vector{Any}(["lol", "loasdl", "aaa"])

	pattern_to_vector(A)
end

# ╔═╡ a81fc6f2-4e35-4f0e-8fa2-e1e75ee58065
# Define a function to predict the tag based on user input
function predict_tag(input_text)
	lol = split(input_text, " ")
	println(lol)
    # Convert the input text into a bag-of-words vector
    input_vec = pattern_to_vector(split(input_text))
	println(input_vec)
    # Pass the input vector through the model to get the predicted tag
    scores = Flux.softmax(model(input_vec))
    # Find the index of the highest score
    max_score, tag_index = findmax(scores)
	println(max_score)
    # Return the corresponding tag
    return tags[tag_index]
end

# ╔═╡ 4407d5bf-0b90-4511-b25b-c2117afab5d3
println(predict_tag("asdasd asdasd asyour name?"))
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "8ffcd228d32dda47b5c1ec8ff3e730b4847e527e"
